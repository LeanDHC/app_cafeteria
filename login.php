...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare("SELECT id_cliente, nome, senha FROM clientes WHERE email = :email");
    ...
    if ($cliente && $cliente['senha'] === $senha) {
        // Armazena o nome do cliente na sessão para uso em `index.php`
        $_SESSION['id_cliente'] = $cliente['id_cliente'];
        $_SESSION['nome_cliente'] = $cliente['nome'];
        header("Location: index.php"); // Redireciona após login bem-sucedido
        exit();
    } else {
